...
export const PATCH = handler(async ({ request, params }) => {
  requireUser(request, 'admin');
  const row = db.prepare('SELECT id FROM proposals WHERE id = ?').get(params.id);
  if (!row) return json({ error: 'Proposta não encontrada' }, 404);
  ...
  db.prepare(`UPDATE proposals SET ${sets}, updated_at = datetime('now') WHERE id = @id`).run({ ...updates, id: row.id });
  return json({ ok: true });
});
